...
Double_t TGenDVCS::XSecSum(int opt)
{
  ...
  //20190426(start)
  //temporary
  // cout<<"TGenDVCS::XsecSum's fe_Px : "<<fe->Px()<<",\t fe_Py : "<<fe->Py()<<",\t fe_Pz : "<<fe->Pz()<<endl;
  // cout<<"TGenDVCS::XsecSum's fq_Px : "<<fq->Px()<<",\t fq_Py : "<<fq->Py()<<",\t fq_Pz : "<<fq->Pz()<<endl;
  // cout<<"TGenDVCS::XsecSum's fg_Px : "<<fg->Px()<<",\t fg_Py : "<<fg->Py()<<",\t fg_Pz : "<<fg->Pz()<<endl;
  // cout<<"TGenDVCS::XSecSum's phi : "<<fphi*TMath::RadToDeg()<<endl;
  //20190426(finish)
...
{ 
  ...
  //20190426(start)
  //test
  // Double_t angle=-(q.Vect()).Angle(oz);// To change electron from the left side to the right side of the beam-line (beam direction perspective)
  Double_t angle=(q.Vect()).Angle(oz);//
  //20190426(finish)
  //20190412(finish)
  ...
  // fp->RotateZ(fphi);
  // fg->RotateZ(fphi);
  fp->RotateZ(fphi + TMath::Pi());
  fg->RotateZ(fphi + TMath::Pi());
  //20190426(finish)
  ...
  //20190226(start)
  // To change photon from the right side to the left side of the beam-line (beam direction perspective)
  // fp->RotateZ(TMath::Pi());
  // fg->RotateZ(TMath::Pi());
  //20190226(finish)                                                                      
  //20190426(start)
  //temporary
  //    cout<<"TGenDVCS::Compute2Bodys phi : "<<fphi*TMath::RadToDeg()<<endl;
  // TLorentzVector eini_test = *feini;
  // TLorentzVector pini_test = *fpini;
  // TLorentzVector e_test = *fe;
  // TLorentzVector q_test = *fq;
  // TLorentzVector p_test = *fp;
  // TLorentzVector g_test = *fg;
  // cout<<"e + p      Px : "<<(eini_test+pini_test).Px()<<",\t Py : "<<(eini_test+pini_test).Py()<<",\t Pz : "<<(eini_test+pini_test).Pz()<<",\t E : "<<(eini_test+pini_test).E()<<endl;
  // cout<<"e'+ q + p  Px : "<<(e_test+q_test+pini_test).Px()<<",\t Py : "<<(e_test+q_test+pini_test).Py()<<",\t Pz : "<<(e_test+q_test+pini_test).Pz()<<",\t E : "<<(e_test+q_test+pini_test).E()<<endl;
  // cout<<"e'+ p'+ g, Px : "<<(e_test+p_test+g_test).Px()<<",\t Py : "<<(e_test+p_test+g_test).Py()<<",\t Pz : "<<(e_test+p_test+g_test).Pz()<<",\t E : "<<(e_test+p_test+g_test).E()<<endl;
  //20190426(finish)
