...
void TGenPi0::IntRCBef(void)
{
  ...
  Double_t deltaE = fQ2/(2.*fm*fxb);
  Double_t Elim=(4*deltaE+4*TMath::Sqrt(deltaE*deltaE+fQ2))/8.; //minimal incident energy in order to make the event possible 

  fRadCor=kTRUE;
  Double_t nu=2.*0.007297352533*(TMath::Log(fQ2/TMath::Power(0.000510998902,2))-1.)/TMath::Pi(); 
  double_t rand_lim=TMath::Power(feini->E()-Elim,nu/2.)*0.999; //maximal value for the random number to not go below Elim
  
  //Double_t eel=(feini->E())*(1-TMath::Power(fRan->Rndm(),1./(nu/2.)));
  Double_t eel=(feini->E())*(1-TMath::Power(fRan->Uniform(0,rand_lim),1./(nu/2.)));
...
//_____________________________________________________________________________
Double_t TGenPi0::XS_pi0(void)
{
  // Returns the cross section for pi0 electroproduction based on a parameterization 
  // fitted and used by CLAS experiment
  
  Double_t yy=fQ2/(2.*fm*fxb)/(feini->E()); 
  Double_t gamma_f=TMath::Power(yy*fxb*fm,2)/fQ2;
  Double_t eps=(1-yy-gamma_f)/(1-yy+yy*yy/2.+gamma_f);

  Double_t factor=0.007297352533*yy*yy*(1-fxb)/2./TMath::Pi()/fxb/fQ2/(1-eps)/2./TMath::Pi();
  Double_t x_dep=2*1.1*TMath::Log(fxb);
   
  Double_t sig_t=846.413*TMath::Exp(-x_dep*0.450476*ft)/TMath::Power(fQ2+1.08767,1.69193); 
  Double_t sig_l=1396.32*TMath::Exp(-x_dep*0.690941*ft)*fQ2/TMath::Power(fQ2+1.08767,1.69193); 
  Double_t sig_tt=-5360.33*(-ft+ftmin)*TMath::Exp(-x_dep*1.01146*ft)/TMath::Power(fQ2+1.08767,1.69193);
  Double_t sig_tl=673.385*TMath::Sqrt(-ft+ftmin)*TMath::Exp(-x_dep*1.00509*ft)/TMath::Power(fQ2+1.08767,1.69193);

  fSigmaTot=factor*(sig_t+eps*sig_l+eps*sig_tt*TMath::Cos(2*fphi)+TMath::Sqrt(2*eps*(1+eps))*sig_tl*TMath::Cos(fphi));


    // cout<<"  yy ="<<yy<<"    gamma = "<<gamma<<"    eps = "<<eps<<"    factor = "<<factor<<"  x_dep = "<<x_dep<<endl;
    // cout<<"sig_tl = "<<sig_tl<<endl;
    // cout<<"ft = "<<ft<<"    ftmin = "<<ftmin<<"    fQ2 = "<<fQ2<<endl;
  return fSigmaTot;
