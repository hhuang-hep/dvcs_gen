...
#include "/group/nps/hhuang/software/NPS_SOFT/TDVCSEvent.h"
#include "/group/nps/hhuang/software/dvcs_gen/TGenGeo.h"
#include "/group/nps/hhuang/software/dvcs_gen/TGenPi0.h"
#include "/group/nps/hhuang/software/dvcs_gen/TGenDVCS.h"
#include "/group/nps/hhuang/software/dvcs_gen/TGVKelly.h"
#include "/group/nps/hhuang/software/dvcs_gen/TGenBase.h"
...
int main()
{
    ...
    Double_t HRS_angle=*db->GetEntry_d("SIMU_param_HMSangle",run_number);
    Double_t HRS_mom=*db->GetEntry_d("SIMU_param_HMSmomentum",run_number);
    
    double targ_length = 10; //cm 
    double targ_offs = 0; // Probably cm/dens
    double targ_dens = 0.07080;
    ...
    TFile *file_out = new TFile("test_DVCS_gen_Vz.root", "recreate");
    ...
    TH1F *h_Vx = new TH1F("h_Vx", ";Vertex x [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vy = new TH1F("h_Vy", ";Vertex y [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vz = new TH1F("h_Vz", ";Vertex z [cm];Normalized counts", 100, -10, 10);

    TH1F *h_Vx_ELossCorr = new TH1F("h_Vx_ELossCorr", ";Vertex x [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vy_ELossCorr = new TH1F("h_Vy_ELossCorr", ";Vertex y [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vz_ELossCorr = new TH1F("h_Vz_ELossCorr", ";Vertex z [cm];Normalized counts", 100, -10, 10);

    TH1F *h_Vx_computeE = new TH1F("h_Vx_computeE", ";Vertex x [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vy_computeE = new TH1F("h_Vy_computeE", ";Vertex y [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vz_computeE = new TH1F("h_Vz_computeE", ";Vertex z [cm];Normalized counts", 100, -10, 10);
    TH2F *hh_eth_ephi_computeE = new TH2F("hh_data_HMSangle_computeE", "Electron #theta vs. #phi;#phi [rad];#theta [rad]", 100, 3.1, 3.2, 100, 0.1, 0.4);

    TH1F *h_Vx_HMScut = new TH1F("h_Vx_HMScut", ";Vertex x [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vy_HMScut = new TH1F("h_Vy_HMScut", ";Vertex y [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vz_HMScut = new TH1F("h_Vz_HMScut", ";Vertex z [cm];Normalized counts", 100, -10, 10);
    TH2F *hh_eth_ephi_HMScut = new TH2F("hh_data_HMSangle_HMScut", "Electron #theta vs. #phi;#phi [rad];#theta [rad]", 100, 3.1, 3.2, 100, 0.1, 0.4);

    TH1F *h_Vx_HMScut_NPScut = new TH1F("h_Vx_HMScut_NPScut", ";Vertex x [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vy_HMScut_NPScut = new TH1F("h_Vy_HMScut_NPScut", ";Vertex y [cm];Normalized counts", 100, -0.5, 0.5);
    TH1F *h_Vz_HMScut_NPScut = new TH1F("h_Vz_HMScut_NPScut", ";Vertex z [cm];Normalized counts", 100, -10, 10);
    TH2F *hh_eth_ephi_HMScut_NPScut = new TH2F("hh_data_HMSangle_HMScut_NPScut", "Electron #theta vs. #phi;#phi [rad];#theta [rad]", 100, 3.1, 3.2, 100, 0.1, 0.4);

    for( int i = 0; i < nsim; i++ ){
        if (i % 1000 == 0) cout << i << "/" << nsim << endl;
        gEv->GenerateVertex();//Generates the vertex

        h_Vx->Fill(gEv->GetVertex()->x());
        h_Vy->Fill(gEv->GetVertex()->y());
        h_Vz->Fill(gEv->GetVertex()->z());
        ...
        h_Vx_ELossCorr->Fill(gEv->GetVertex()->x());
        h_Vy_ELossCorr->Fill(gEv->GetVertex()->y());
        h_Vz_ELossCorr->Fill(gEv->GetVertex()->z());
        ...
        if(gEv->ComputeElectron()){//Compute the scattered electron 4-vector
            ...
            h_Vx_computeE->Fill(gEv->GetVertex()->x());
            h_Vy_computeE->Fill(gEv->GetVertex()->y());
            h_Vz_computeE->Fill(gEv->GetVertex()->z());
            hh_eth_ephi_computeE->Fill(L_em_scat.Phi(), L_em_scat.Theta());

            // cout<<"phi = "<<L_em_scat.Phi()<<" rad; "<<"theta = "<<L_em_scat.Theta()<<" rad"<<endl;
            ...
            // cout<<"phi = "<<L_final_phot.Phi()*TMath::RadToDeg()<<" deg; "<<"theta = "<<L_final_phot.Theta()*TMath::RadToDeg()<<" deg"<<endl;
            ...
            if(gEv->HitsSpectro(gEv->GetScatteredElectron()) ){
                ...
                h_Vx_HMScut->Fill(gEv->GetVertex()->x());
                h_Vy_HMScut->Fill(gEv->GetVertex()->y());
                h_Vz_HMScut->Fill(gEv->GetVertex()->z());
                hh_eth_ephi_HMScut->Fill(L_em_scat.Phi(), L_em_scat.Theta());

                if(gEv->HitsCalo(gEv->GetFinalPhoton()))
                {
                    h_Vx_HMScut_NPScut->Fill(gEv->GetVertex()->x());
                    h_Vy_HMScut_NPScut->Fill(gEv->GetVertex()->y());
                    h_Vz_HMScut_NPScut->Fill(gEv->GetVertex()->z());
                    hh_eth_ephi_HMScut_NPScut->Fill(L_em_scat.Phi(), L_em_scat.Theta());
    ...
    h_Vx_ELossCorr->Write();
    h_Vy_ELossCorr->Write();
    h_Vz_ELossCorr->Write();

    h_Vx_computeE->Write();
    h_Vy_computeE->Write();
    h_Vz_computeE->Write();
    hh_eth_ephi_computeE->Write();

    h_Vx_HMScut->Write();
    h_Vy_HMScut->Write();
    h_Vz_HMScut->Write();
    hh_eth_ephi_HMScut->Write();

    h_Vx_HMScut_NPScut->Write();
    h_Vy_HMScut_NPScut->Write();
    h_Vz_HMScut_NPScut->Write();
    hh_eth_ephi_HMScut_NPScut->Write();
